...
"""
Neste primeiro exericio, pretendemos justificar uma cadeia de caracteres, que nao seja vazia, utilizando um
...
"""
Com este programa pretendemos obter os resultados de uma determinada eleicao, utilizando o metodo de
Hondt (um modelo matematico utilizado para converter votos em mandatos em sistemas eleitorais de
representacao proporcional). Para isso, o argumento usado para a funcao tera os nomes dos territorios,
dos partidos, o numero de votos de cada partido e o numero de deputados num determinado territorio.
"""
...
def calcula_quocientes(circulo, n):
    '''
    Esta funcao recebe um dicionario com os votos apurados num circulo (com pelo menos um partido) e
    um inteiro positivo representando o numero de deputados e devolve o dicionario com as mesmas
    chaves do dicionario argumento (correspondente a partidos) contendo a lista (de comprimento igual
    ao numero de deputados) com os quocientes calculados com o metodo de Hondt ordenados em ordem
    decrescente. Esta funcao nao deve de alterar o dicionario que e passado como argumento de entrada.
    --------------------------------------
...
def atribui_mandatos(circulo, n):
    ...
    numero de deputados, e devolve a lista ordenada de tamanho igual ao numero de deputados contendo
...
def obtem_partidos(eleicoes):
    ...
    circulos eleitorais como descrito, e devolve a lista por ordem alfabetica com o nome de todos
...
def obtem_resultado_eleicoes(eleicoes):
    '''
    Esta funcao recebe um dicionario com as informacoes das eleicoes num territorio com diversos
    circulos eleitorais e devolve a lista ordenada de comprimento igual ao numero total de
    partidos com os resultados das eleicoes.
    -----------------------
    {dicionario} -> {lista}
    '''
    try: # Caso de erro para levantar ValueError
        ...
        for c in eleicoes:
            if type(c) != str or type(eleicoes[c]) != dict or len(eleicoes[c]) != 2 or not "votos" in eleicoes[c] or not "deputados" in eleicoes[c]: # Verifica o tipo e se so existe a chave deputados e votos dentro do dicionario
                raise ValueError("obtem_resultado_eleicoes: argumento invalido")
        ...
        res.sort(key=lambda x:(x[1],x[2]), reverse=True) # Ordena por ordem decrescente
        return res
    except:
        raise ValueError("obtem_resultado_eleicoes: argumento invalido")
...
"""
Esta terceira tarefa aborda o problema da solucao de sistemas de equacoes lineares atravez do metodo
de Jacobi (O metodo de Jacobi e um metodo iterativo que resolve o membro esquerdo da expressão em
ordem a x ao usar o método resultante da iteração anterior no membro direito).
"""
...
def produto_interno(tup1, tup2):
    '''
    Esta funcao recebe dois tuplos de numeros com a mesma dimensao representando dois vetores e retorna
    o resultado do produto interno desses 2 vetores.
    -------------------------
    {tuplo x tuplo} -> {real}
    '''
    soma = 0.0
...
def verifica_convergencia(tup1, tup2, tup3, real):
    ...
    constituido por um conjunto de tuplos cada um representando uma linha da matriz quadrada A, e
    os outros dois tuplos de entrada contem valores representando respetivamente o vetor de
    constantes e a solucao atual. O valor real de entrada indica a precisao pretendida. Este
    retorna True no caso do valor abosulto for inferior a precisao e False caso contrario.
    --------------------------------------------
...
def retira_zeros_diagonal(tup1, tup2):
    ...
    Esta retorna uma nova matriz em que em todas as linhas que tenha algum 0 na diagonal troquem
    pela primeira que tenha um numero diferente de 0.
    ----------------------------------
...
def eh_diagonal_dominante(tup):
    '''
    Esta funcao recebe um tuplo de tuplos representando uma matriz quadrada no mesmo formato das
    funcoes anteriores. Esta verifica se a diagonal da matriz e dominante ou nao.
    ---------------------
    {tuplo} -> {booleano}
    '''
...
def resolve_sistema(tup1, tup2, real):
    '''
    Esta funcao recebe um tuplo de tuplos representando uma matriz quadrada correspondente aos
    coeficientes das equacoes do sistema, um tuplo de numeros representando o vetor das
    ...
    # Verifica o tipo dos argumentos, se o tamanho das constantes ou da matriz e zero, se tem igual numero de constantes e de linhas da matriz e se a precisao e superior a zero

    for i in range(len(tup1)):
        if type(tup2[i]) != float and type(tup2[i]) != int: # Verifica o tipo dos numeros dentro das constantes
            raise ValueError("resolve_sistema: argumentos invalidos")
    ...
    matriz, const = retira_zeros_diagonal(tup1, tup2) # Retira os zeros da diagonal para poder aplicar o metodo
